...
func TestCheck_HTTP(t *testing.T) {
	...
	runner := NewCheckRunner(logger, client, 0, 0, &tls.Config{}, 1, 1, false)
	defer runner.Stop()
...
func TestCheck_MinimumInterval(t *testing.T) {
	...
	runner := NewCheckRunner(logger, client, 0, minimumInterval, &tls.Config{}, 0, 0, false)
	defer runner.Stop()
...
func TestCheck_NoFlapping(t *testing.T) {
	...
	minimumInterval := 2 * time.Second
	runner := NewCheckRunner(logger, client, 0, minimumInterval, &tls.Config{}, 2, 2, false)
	defer runner.Stop()
