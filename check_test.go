...
func TestCheck_MinimumInterval(t *testing.T) {
	...
	runner := NewCheckRunner(logger, client, 0, minimumInterval, &tls.Config{}, 0, 0, false, 0)
	defer runner.Stop()
...
func TestCheck_NoFlapping(t *testing.T) {
	...
	minimumInterval := 2 * time.Second
	runner := NewCheckRunner(logger, client, 0, minimumInterval, &tls.Config{}, 2, 2, false, 0)
	defer runner.Stop()
